...
s3_bucket=`$aws ec2 describe-tags --filters Name=key,Value=s3_bucket | jq --raw-output '.Tags[0].Value'`
s3_output_path=`$aws ec2 describe-tags --filters Name=key,Value=s3_out | jq --raw-output '.Tags[1].Value'`
